...
def servicios(request):
    datos_cho = Chofer.objects.all()
    datos_transfer = Transfer.objects.all()
    
    datos = {
        "datos_cho": datos_cho,
        "datos_transfer": datos_transfer,
    }
...
def reserva(request):
    if request.method == 'POST':
        ...
        if ticket_form.is_valid() and detalle_form.is_valid():
            ...
            precio_total = PRECIO_POR_ASIENTO * detalle_ticket.asientosReserva
            ticket.precio = precio_total
            ticket.save()
